 ...
 */
robot_command_thread::robot_command_thread(const std::string node_name, const std::string& robot_ip):rclcpp::Node(node_name,
    ...
    /********************************尝试使用SDK库连接机械臂******************************************/
    // Use provided IP or fall back to default
    _controller_ip = robot_ip.empty() ? CONTROLLER_IP : robot_ip;
    ...
    while(connect_count <_connect_retry_SDK){
        error_t returncode = _ptr_robot->RPCXmlOnly(_controller_ip.c_str());
        if(returncode !=0){
            connect_count++;
